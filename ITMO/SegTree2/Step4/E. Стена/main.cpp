...
struct segtree
{
    ...
    op NO_OP = {.min = INT_MAX, .max = INT_MIN};

    void init(int n)
    {
        size = 1;
        while (size < n) size *= 2;
        tree.assign(2LL * size, {.modify = NO_OP, .value = 0});
    }

    auto _modify(op const& a, op const& b) const -> op
    {
        ...
        if (b.min <= a.min) return {.min = a.min, .max = b.max};
        return {.min = b.min, .max = a.max};
    }

    static auto _modify(int a, op const& b) -> int
    {
    ...
    void collect(vector<int>& result, int x, int lx, int rx)
    {
        if (rx - lx == 1)
        {
            if (std::cmp_less(lx, result.size()))
            {
...
auto main() -> int
{
    ...
    while (k--)
    {
        ...
        if (t == 1)
        {
            st.update(l, r + 1, {.min = h, .max = INT_MAX});
        }
        else
        {
            st.update(l, r + 1, {.min = INT_MIN, .max = h});
        }
    }
